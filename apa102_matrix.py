...
class Apa102Matrix(metaclass=Singleton):
    def __init__(self, num_rows=16, num_cols=16, color_type=ColorType.bgr, wire_mode=WireMode.zig_zag, origin=Origin.bottom_left, orientation=Orientation.horizontally):
        ...
        self.__end_frame = [0xff] * ((self.num_leds + 15) // (2 * 8)) # end frame is >= (n/2) bits of 1, where n is the number of LEDs
        self.__rgb_buffer = np.zeros((self.num_rows, self.num_cols, 3), dtype=np.uint8) #3 for red, green and blue
        self.__led_frame_start = 0b11100000
        
        # setup datastructures for fast lookup of led
        # led index for given coordinate
        self.__pixel_coord_to_led_index = np.zeros((self.num_rows, self.num_cols),dtype=np.int)
        self.__virtual_to_physical_byte_indices = np.zeros((self.num_rows, self.num_cols, 4),dtype=np.int)
    ...
    def populate_gamma_array(self):
        for i in np.arange(256, dtype=np.uint8):
            self.__gamma8[i] = (255 * ((i/255)**(self.gamma)) + 0.5).astype(np.uint8)
    ...
    def populate_pixel_to_led_index_datastructure(self):
        ...
        for i in range(self.num_rows):
            for j in range(self.num_cols):
                ...
                self.__virtual_to_physical_byte_indices[i,j] = [led_index, led_index + red, led_index + green, led_index + blue]
            
    def __str__(self):
        header = "APA102-matrix configuration: rows: {} cols: {} colortype: {} wiremode: {} origin: {} orientation {}\n".format(self.num_rows, self.num_cols,\
                                   self.color_type, self.wire_mode, self.origin, self.orientation)
    ...
    def get_brightness_array(self):
        ...
        if brightness > 31:
            brightness = 31
        led_frame_first_byte = (brightness & ~self.__led_frame_start) | self.__led_frame_start
        ret = np.array([led_frame_first_byte] * self.num_leds, dtype=np.uint8).reshape((self.num_rows, self.num_cols, 1))
        return ret

    def gamma_correct(self):
        for x in np.nditer(self.__rgb_buffer, op_flags=['readwrite'], flags=['external_loop','buffered'], order='F'):
            x[...] = self.__gamma8[x]
    
    def show(self, gamma=False):
        if gamma:
            self.gamma_correct()
        complete_led_frames = np.concatenate((self.get_brightness_array(),self.__rgb_buffer), axis=2)
        to_send = self.__start_frame + complete_led_frames.take(self.__virtual_to_physical_byte_indices).flatten().tolist() + self.__end_frame
...
##    def show_list_of_rgb_tuples(self, rgb_tuples, gamma=False):
...
##            except TypeError:
##                return
##            self.set_pixel_at_index(i, red, green, blue)
##        self.show(gamma)
    ...
    def show_test_pattern(self, gamma=False):
        ...
        self.__rgb_buffer[0:self.num_cols/4*1,:,0:3] = self.__rgb_buffer[0:4,:,0:3] + np.resize(values,(3,self.num_cols)).transpose()
    ...
    def benchmark(self,gamma=False):
        ...
        for i in range(repeat):
            ...
            diff = end - start
            total = total + diff
        print("{:.2f}s for {} iterations. {:d} refreshs per second".format(total, repeat, int(repeat/total)))
        self.clear_rgb_buffer()
        self.show()
